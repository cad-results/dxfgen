...
for line in lines:
    ...
    # Strip whitespace from each field to handle CSV with spaces after commas
    line_data = [item.strip() for item in line.split(col_delm)]
    ...
    if data[index]['type']=="L":
        ...
        if data[index]['show_line']=="1":
            hatch = msp.add_hatch(color= hatch_color,
                dxfattribs={'layer': data[index]['layer']})
            # add boundary path (updated for ezdxf 1.x API)
            ...
            hatch.paths.add_polyline_path(line_points, is_closed=True)
...
for line_data in data:
    if line_data['text']=="NO TEXT" or line_data['show_line']=="0":
        ...
        if x_values!=[] and y_values!=[]:
            ...
            Xtxt=meanX
            Ytxt=minY - txt_sp
            # Updated for ezdxf 1.x API - use dxfattribs for position and alignment
            msp.add_text(line_data['text'], dxfattribs={
